...
def load_merged_explanation(file_path):
    """Load the token to neuron mapping from the CSV file."""
    df = pd.read_csv(file_path, sep=",", engine="python", on_bad_lines="skip")
    return df


def load_sae_decoder_vectors(file_path):
    """Load the SAE decoder vectors from the PyTorch file."""
    decoder_dict = torch.load(file_path)
    return decoder_dict
...
def perturb_salient_embeddings_orthogonal_sae(cls_embeddings, sentence_ids, merged_explanation_df, sae_decoder_vectors):
    """Remove the SAE concept direction from CLS token embeddings using orthogonal projection."""
    ...
    for i, sentence_id in enumerate(sentence_ids):
        ...
        if len(salient_token_df) == 0:
            print(f"Warning: No salient token found for sentence {sentence_id}")
            continue
            
        # For SAE, we get the neuron index instead of vector index
        neuron_idx = salient_token_df.iloc[0]['neuron_idx']
        
        # Skip if neuron_idx is not available
        if neuron_idx == "NA" or neuron_idx not in sae_decoder_vectors:
            print(f"Warning: Neuron index {neuron_idx} not found in decoder vectors")
            continue
        
        # Get the corresponding decoder vector (concept vector)
        salient_concept_vector = sae_decoder_vectors[neuron_idx]
        
        if isinstance(salient_concept_vector, list):
            salient_concept_vector = np.array(salient_concept_vector)
        elif isinstance(salient_concept_vector, torch.Tensor):
            salient_concept_vector = salient_concept_vector.cpu().numpy()
        
        # Remove the concept direction using orthogonal projection
        perturbed_cls_embeddings[i] = project_orthogonal(perturbed_cls_embeddings[i], salient_concept_vector)
        perturbed_indices.append(i)
    
    print(f"Applied orthogonal projection to remove SAE concept directions from {len(perturbed_indices)} embeddings")
...
def perturb_salient_embeddings_orthogonal_random(cls_embeddings, perturbed_indices, seed=42):
    ...
    For each perturbed embedding, select a random vector and project orthogonally to it.
    """
    random.seed(seed)
    np.random.seed(seed)
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description="Train and evaluate classifiers on SAE-perturbed embeddings")
    ...
    parser.add_argument('--merged-explanation-file',
                        type=str,
                        required=True,
                        help='Path to the merged explanations CSV file.')
    
    parser.add_argument('--sae-decoder-vectors',
                        type=str,
                        required=True,
                        help='Path to the SAE decoder vectors file.')
    
    parser.add_argument('--model-name',
                        type=str,
                        required=True,
                        help='The name or path of the language model.')
    
    parser.add_argument('--output-dir',
                        type=str,
                        default='sae_classifier_results/',
...
def main():
    ...
    # Load dataset, explanation file, and SAE decoder vectors
    sentences = get_dataset(args.dataset_path)
    merged_explanation_df = load_merged_explanation(args.merged_explanation_file)
    sae_decoder_vectors = load_sae_decoder_vectors(args.sae_decoder_vectors)
    ...
    print(f"Loaded {len(sae_decoder_vectors)} SAE decoder vectors")
    
    # Get sentence IDs from explanation file
    sentence_ids = sorted(merged_explanation_df['sentence_index'].unique())
    print(f"Found {len(sentence_ids)} unique sentence IDs in explanations")

    max_sentences = min(len(sentences), len(ground_truth_labels), len(sentence_ids))
    ...
    # Create perturbed CLS token embeddings using SAE concept vectors
    print("\nCreating perturbed CLS token embeddings using SAE concept vectors...")
    perturbed_cls_embeddings, perturbed_indices = perturb_salient_embeddings_orthogonal_sae(
        cls_embeddings, sentence_ids, merged_explanation_df, sae_decoder_vectors
    )
    ...
    embedding_names = ["Normal CLS", "SAE Perturbed CLS", "Random Perturbed CLS"]
    ...
    print(f"SAE perturbation effect: {results['Normal CLS']['mean_accuracy'] - results['SAE Perturbed CLS']['mean_accuracy']:.4f}")
    print(f"Random perturbation effect: {results['Normal CLS']['mean_accuracy'] - results['Random Perturbed CLS']['mean_accuracy']:.4f}")
    print(f"SAE vs Random difference: {results['Random Perturbed CLS']['mean_accuracy'] - results['SAE Perturbed CLS']['mean_accuracy']:.4f}")
    ...
    summary = {
        ...
        "perturbation_method": "sae_orthogonal_projection",
    ...
    summary["sae_perturbation_effect"] = float(results['Normal CLS']['mean_accuracy'] - results['SAE Perturbed CLS']['mean_accuracy'])
    summary["random_perturbation_effect"] = float(results['Normal CLS']['mean_accuracy'] - results['Random Perturbed CLS']['mean_accuracy'])
    summary["sae_vs_random_difference"] = float(results['Random Perturbed CLS']['mean_accuracy'] - results['SAE Perturbed CLS']['mean_accuracy'])
    
    with open(os.path.join(args.output_dir, "sae_comparison_summary.json"), 'w') as f:
