...
def train_and_evaluate_individual_models(embeddings_list, labels, embedding_names, 
                                      n_folds=5, batch_size=32, epochs=10, lr=0.001, 
                                      device="cuda", patience=3, min_delta=0.001, seed=42):
    """
    Train a separate model on each embedding type and evaluate it using the same CV splits.
    
    Args:
        embeddings_list: List of embedding arrays
        labels: Single label array (same for all embeddings)
        embedding_names: Names for each embedding type
    ...
    # Create consistent CV splits for all embedding types
    cv = StratifiedKFold(n_splits=n_folds, shuffle=True, random_state=seed)
    splits = [(train_idx, val_idx) for train_idx, val_idx in cv.split(embeddings_list[0], labels)]
    
    for emb_idx, (embeddings, name) in enumerate(zip(embeddings_list, embedding_names)):
        print(f"\nTraining and evaluating on {name} embeddings...")
        
        # Convert numpy arrays to PyTorch tensors
        embeddings_tensor = torch.FloatTensor(embeddings).to(device)  # Fixed: Added .to(device)
        labels_tensor = torch.LongTensor(labels).to(device)           # Fixed: Added .to(device)
        
        for fold_num, (train_idx, val_idx) in enumerate(splits, 1):
            # Split data according to predefined splits
            X_train, X_val = embeddings_tensor[train_idx], embeddings_tensor[val_idx]
            ...
            for epoch in range(epochs):
                ...
                if val_acc > best_val_acc + min_delta:
                    ...
                    best_model_state = copy.deepcopy(model.state_dict())  # Fixed: Use deep copy
                    epochs_no_improve = 0
                else:
                    epochs_no_improve += 1
            ...
            val_acc = accuracy_score(val_true, val_preds)
            all_results[name]["fold_accuracies"].append(val_acc)
            all_results[name]["predictions"].extend(val_preds)
            all_results[name]["true_labels"].extend(val_true)
            
            print(f"Fold {fold_num}/{n_folds}: {name} - Accuracy: {val_acc:.4f} (after {last_epoch} epochs)")
...
def main():
    ...
    # Create lists of embeddings for each type
    embeddings_list = [cls_embeddings, perturbed_cls_embeddings, random_perturbed_cls_embeddings]
    
    # Train and evaluate individual models (Fixed: Use single labels array)
    print("\nTraining and evaluating individual models on each embedding type:")
    results = train_and_evaluate_individual_models(
        embeddings_list, labels, embedding_names,
    ...
    with open(os.path.join(args.output_dir, "individual_models_comparison_summary.json"), 'w') as f:
