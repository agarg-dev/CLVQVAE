...
# # # --- MODIFICATION: Using a simpler Logistic Regression probe ---
# # class LogisticRegressionClassifier(nn.Module):
# #     """A simple logistic regression classifier, better for probing high-dimensional embeddings."""
...
#     """A more robust MLP with Batch Normalization for better training stability."""
#     def __init__(self, input_dim, num_classes=2):
...
# def extract_last_token_embeddings(model, tokenizer, sentences, device, layer_idx=-1, batch_size=16):
#     """Extract the last token's embeddings in batches to save memory."""
#     all_embeddings = []
#     print("Extracting last token embeddings in batches...")
...
#         sequence_lengths = (inputs['attention_mask'].sum(dim=1) - 1).to(device)
#         last_token_embeddings = layer_output[torch.arange(layer_output.size(0)), sequence_lengths, :]
        
#         all_embeddings.append(last_token_embeddings.cpu().numpy())
...
# def perturb_salient_embeddings_orthogonal(embeddings, sentence_ids, merged_explanation_df, codebook_vectors):
...
#                            embedding_names, n_folds=5, batch_size=32, epochs=10, lr=0.0001, 
...
#     all_results = {name: {"fold_accuracies": []} for name in embedding_names}
...
#         fold_results = ", ".join([f"{name}: {all_results[name]['fold_accuracies'][-1]:.4f}" for name in embedding_names])
...
#     parser = argparse.ArgumentParser(description="Train and evaluate classifiers on perturbed embeddings for decoder models.")
...
#     parser.add_argument('--layer-idx', type=int, default=-1, help="Layer index for embeddings. Default is -1 (last layer).")
...
#     parser.add_argument('--batch-size', type=int, default=16, help='Batch size for model inference to prevent OOM errors.')
...
#     labels = load_ground_truth_labels(args.ground_truth_file)
    
#     print(f"Loaded {len(sentences)} sentences and {len(labels)} ground truth labels")
    
#     if len(sentences) != len(labels):
#         min_len = min(len(sentences), len(labels))
#         print(f"Warning: Mismatch in sentence and label count. Truncating to {min_len}.")
#         sentences = sentences[:min_len]
#         labels = labels[:min_len]

#     sentence_ids = sorted(list(set(merged_explanation_df['sentence_index'])), key=int)
    
#     tokenizer = AutoTokenizer.from_pretrained(args.model_name)
    
#     print(f"Loading DECODER model: {args.model_name}")
...
#     original_embeddings = extract_last_token_embeddings(
#         model, tokenizer, sentences, device, layer_idx=args.layer_idx, batch_size=args.batch_size
#     )
...
#         original_embeddings, sentence_ids, merged_explanation_df, codebook_vectors
#     )
...
#     print(f"Specific perturbation effect: {results['Normal']['mean_accuracy'] - results['Perturbed']['mean_accuracy']:.4f}")
#     print(f"Random perturbation effect: {results['Normal']['mean_accuracy'] - results['Random Perturbed']['mean_accuracy']:.4f}")
#     print(f"Specific vs Random difference: {results['Random Perturbed']['mean_accuracy'] - results['Perturbed']['mean_accuracy']:.4f}")
...
#     summary["specific_perturbation_effect"] = float(results['Normal']['mean_accuracy'] - results['Perturbed']['mean_accuracy'])
#     summary["random_perturbation_effect"] = float(results['Normal']['mean_accuracy'] - results['Random Perturbed']['mean_accuracy'])
#     summary["specific_vs_random_difference"] = float(results['Random Perturbed']['mean_accuracy'] - results['Perturbed']['mean_accuracy'])
...
# import torch.nn as nn
# import torch.optim as optim
# # --- MODIFICATION: Import the learning rate scheduler ---
...
# def perturb_salient_embeddings_orthogonal(embeddings, sentences, tokenizer, sentence_ids, merged_explanation_df, codebook_vectors):
#     """Remove the concept direction from embeddings using orthogonal projection."""
#     perturbed_embeddings = embeddings.copy()
#     perturbed_indices = []
...
#         # --- MODIFICATION START ---
#         # This block ensures the vector is a NumPy array, which is needed for the .shape attribute and np.dot, etc.
#         if isinstance(salient_codebook_vector, torch.Tensor):
#             salient_codebook_vector = salient_codebook_vector.cpu().numpy()
...
#         # The print statements we added earlier
#         if i < 3:
#             salient_token = salient_token_df.iloc[0]['token']
#             salient_position = salient_token_df.iloc[0]['position_index']

#             current_sentence_text = sentences[i]
            
#             tokens = tokenizer.tokenize(current_sentence_text)
#             last_token = tokens[-1] if tokens else "[N/A]"
#             last_token_position = len(tokens) - 1 if tokens else -1

#             print("\n" + "="*60)
#             print(f"--- Analyzing Sentence (Original ID: {sentence_id}, Batch Index: {i}) ---")
#             print(f"  > Salient Token: '{salient_token}' (at your custom position: {salient_position})")
#             print(f"  > Last Token:    '{last_token}' (at standard position: {last_token_position})")
#             print(f"  > Perturbing last token's embedding with codebook vector for '{salient_token}'")
#             print(f"  > Codebook Vector Details:")
#             print(f"    - Shape: {salient_codebook_vector.shape}") # This will now work
#             print(f"    - First 5 elements: {salient_codebook_vector[:5]}")
#             print("="*60)

#         perturbed_embeddings[i] = project_orthogonal(perturbed_embeddings[i], salient_codebook_vector)
#         perturbed_indices.append(i)
    
#     print(f"\nApplied orthogonal projection to remove concept directions from {len(perturbed_indices)} embeddings")
#     return perturbed_embeddings, perturbed_indices
...
#         criterion = nn.CrossEntropyLoss()
#         optimizer = optim.Adam(model.parameters(), lr=lr)
#         # --- MODIFICATION: Initialize the scheduler ---
...
#             # --- MODIFICATION: Step the scheduler ---
...
#     parser.add_argument('--n-folds', type=int, default=20)
#     parser.add_argument('--batch-size', type=int, default=16, help='Batch size for model inference to prevent OOM errors.')
#     parser.add_argument('--eval-embedding', required=True,help='Path to the ground truth JSON file with labels.')
#     return parser.parse_args()
...
#         embedding_names,
#         n_folds=args.n_folds, epochs=args.epochs, patience=args.patience, 
#         lr=args.lr, min_delta=args.min_delta, device=device, seed=args.seed, batch_size=args.batch_size
#     )
...
#     df = pd.read_csv(file_path, sep=",", engine="python", on_bad_lines="skip")
#     df = df.dropna(subset=["sentence_index", "position_index", "vector_idx"])
...
#     # 1. Build a map of all token embeddings and find the last index for each sentence
...
#     # 2. Extract the embedding for the last token of each required sentence
...
#     print(f"\nTraining with {n_folds}-fold cross-validation...")
#     for fold_num, (train_idx, val_idx) in enumerate(cv.split(train_embeddings, train_labels), 1):
#         X_train, X_val = train_embeddings_tensor[train_idx], train_embeddings_tensor[val_idx]
#         y_train, y_val = train_labels_tensor[train_idx], train_labels_tensor[val_idx]
...
#         other_val_loaders = []
#         for other_embs, other_lbls in zip(other_embeddings_list, other_labels_list):
#             X_other_val = torch.FloatTensor(other_embs)[val_idx]
#             y_other_val = torch.LongTensor(other_lbls)[val_idx]
#             other_val_loaders.append(DataLoader(TensorDataset(X_other_val, y_other_val), batch_size=batch_size))
        
#         model = SimpleClassifier(input_dim=train_embeddings.shape[1], num_classes=num_classes).to(device)
#         criterion = nn.CrossEntropyLoss()
#         optimizer = optim.Adam(model.parameters(), lr=lr)
...
#         # Evaluate on all embedding types
#         for i, name in enumerate(embedding_names):
#             loader = val_loader if i == 0 else other_val_loaders[i-1]
#             model.eval()
#             preds, true = [], []
#             with torch.no_grad():
...
#             all_results[name]["fold_accuracies"].append(acc)

#         fold_results = ", ".join([f"{name}: {all_results[name]['fold_accuracies'][-1]:.4f}" for name in embedding_names])
#         print(f"Fold {fold_num}/{n_folds}: {fold_results}")
    
#     for name in embedding_names:
#         mean_acc = np.mean(all_results[name]["fold_accuracies"])
#         std_acc = np.std(all_results[name]["fold_accuracies"])
#         print(f"\n{name} Final Results: Mean Accuracy: {mean_acc:.4f} ± {std_acc:.4f}")
#         all_results[name]["mean_accuracy"] = mean_acc
#         all_results[name]["std_accuracy"] = std_acc
...
#     # Arguments for file paths, reflecting your directory structure
#     parser.add_argument('--dataset-path', type=str, required=True)
#     parser.add_argument('--merged-explanation-file', type=str, required=True)
#     parser.add_argument('--codebook-vectors', type=str, required=True)
#     parser.add_argument('--ground-truth-file', type=str, required=True)
...
#     # 2. Load pre-extracted embeddings based on the chosen method
...
def main():
    ...
    # --- MODIFICATION: Load sentences for debugging, but not the tokenizer ---
    sentences = get_dataset(args.dataset_path)
