...
def get_salient_words(filename, method='top-1', attribution_mass=None, include_last_token=False):
    ...
    for sentence_idx, saliency in enumerate(saliencies):
        ...
        if include_last_token:
            tokens_to_sort = saliency
            print(f"Sentence {sentence_idx}: Including all {len(tokens_to_sort)} tokens in analysis.")
        else:
            tokens_to_sort = saliency[:-1]
            print(f"Sentence {sentence_idx}: Ignoring last token. Analyzing {len(tokens_to_sort)} tokens.")

        # sort the saliency in descending order and save original index
        sorted_saliency = sorted(enumerate(tokens_to_sort), key=lambda x: x[1][1], reverse=True)
...
def main():
    ...
    parser.add_argument("--include-last-token", action="store_true", help="Include the last token in the saliency analysis.")

    args = parser.parse_args()

    important_tokens, sentence_id, prediction_class = get_salient_words(args.input_file, args.method, args.attribution_mass, include_last_token=args.include_last_token)
